...
음성 인식은 `faster-whisper`를 사용하며, 전사 진행률을 실시간으로 확인할 수 있습니다. 또한 서버 실행 상태, HTTP 요청, 전사 작업 및 오류를 로그 파일로 기록하여 문제 발생 시 원인을 확인할 수 있도록 구성했습니다.
...
* **[Issue] 프로그램 실행 직후 서버 종료 현상**
  * 강의 전사 프로그램 실행 직후 Flask 서버가 한 차례 종료되는 현상 발생
  * 서버 실행 및 메인 페이지 접속(`GET /`, HTTP 200)까지는 정상적으로 확인됨
  * 브라우저의 `favicon.ico` 요청에서 404 로그가 발생했으나, 해당 요청 자체가 서버 종료의 직접적인 원인인지는 확인되지 않음
  * 서버 종료 원인 확인을 위해 추가적인 오류 로그 및 종료 시점의 traceback 확인이 필요함
...
| PDF 생성  | ReportLab                  |
| 로깅      | Python `logging`           |
| 실행 환경   | Python Virtual Environment |
...
`RotatingFileHandler`를 사용하여 로그 파일 크기를 자동으로 관리합니다.
...
3. 전사 진행률 확인
4. 전사 완료 후 결과 확인
5. TXT / SRT / PDF / DOCX 중 원하는 형식으로 다운로드
...
1. Select an MP3 or MP4 lecture file.
2. Click the upload button.
3. Monitor the real-time transcription progress.
4. Review the transcription result when processing is complete.
5. Download the result as TXT, SRT, PDF, or DOCX.
...
* [x] Server shutdown logging
* [x] HTTP request logging
* [x] Server error logging
* [x] Transcription traceback logging
